...
%res = xlsread('diabetes_data_upload2.csv');
res = xlsread('fy1.csv');
%%  分析数据
...
for i = 1:SearchAgents_no
    fitness(i) = (fitcal(reshape(Positions(i, :), 1, dim), num_hidden, p_train, t_train, net));
end
...
while iter < Max_iteration           % 对迭代次数循环
    ...
    ww=((Max_iteration-2)/Max_iteration)*(w_start-(w_start-w_end)*atan(aa+bb.*iter)*2/pi).^2;
    ...
    for i = 1 : size(Positions, 1)   % 遍历每个狼
        ...
        % 更新 Alpha, Beta, Delta
        if fitness(i) < Alpha_score           % 如果目标函数值小于Alpha狼的目标函数值
            Alpha_score = fitness(i);         % 则将Alpha狼的目标函数值更新为最优目标函数值
            Alpha_pos = Positions(i, :);   % 同时将Alpha狼的位置更新为最优位置
        end
        
        if fitness(i) > Alpha_score && fitness(i) < Beta_score   % 如果目标函数值介于于Alpha狼和Beta狼的目标函数值之间
            Beta_score = fitness(i);                          % 则将Beta狼的目标函数值更新为最优目标函数值
            Beta_pos = Positions(i, :);                    % 同时更新Beta狼的位置
        end
        
        if fitness(i) > Alpha_score && fitness(i) > Beta_score && fitness(i) < Delta_score  % 如果目标函数值介于于Beta狼和Delta狼的目标函数值之间
            Delta_score = fitness(i);                                                 % 则将Delta狼的目标函数值更新为最优目标函数值
    ...
    Worst_pos = Positions(indexSort(SearchAgents_no),:);
    
    New_alpha_pos=Alpha_pos+ww*s*(Alpha_pos-Worst_pos);
    
    New_alpha_fitness=(fitcal(reshape(New_alpha_pos, 1, dim), num_hidden, p_train, t_train, net));
    
    if New_alpha_fitness<Alpha_score
        Alpha_pos=New_alpha_pos;
        Alpha_score=New_alpha_fitness;
    end
    
    New_alpha_pos=ub+lb-s*Alpha_pos;    
    New_alpha_fitness=(fitcal(reshape(New_alpha_pos, 1, dim), num_hidden, p_train, t_train, net));
    if New_alpha_fitness<Alpha_score
        Alpha_pos=New_alpha_pos;
        Alpha_score=New_alpha_fitness;
    ...
    for i = 1:SearchAgents_no
        ...
        New_position1=Positions(i,:)+s*(Alpha_pos-Positions(i,:));
        New_fitness1=(fitcal(reshape(New_position1, 1, dim), num_hidden, p_train, t_train, net));
        
        New_position2=ww*Positions(i,:);
        New_fitness2=(fitcal(reshape(New_position2, 1, dim), num_hidden, p_train, t_train, net));
    ...
    for i = 1:SearchAgents_no     
        ...
        if q<pp
            ...
            New_position1=Positions(randi(SearchAgents_no),:)-rand()*abs(Positions(randi(SearchAgents_no),:)-2*rand()*Positions(i,:));
            New_fitness1=(fitcal(reshape(New_position1, 1, dim), num_hidden, p_train, t_train, net));
            
            % perch on a random tall tree (random site inside group's home range)
            New_position2=(Alpha_pos-mean(Positions))-rand()*((ub-lb)*rand+lb);
            New_fitness2=(fitcal(reshape(New_position2, 1, dim), num_hidden, p_train, t_train, net));
